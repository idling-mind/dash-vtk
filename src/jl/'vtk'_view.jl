...
"""
    'vtk'_view(;kwargs...)
    'vtk'_view(children::Any;kwargs...)
    'vtk'_view(children_maker::Function;kwargs...)
...
"""
function 'vtk'_view(; kwargs...)
        available_props = Symbol[:children, :id, :background, :cameraParallelProjection, :cameraPosition, :cameraViewUp, :className, :clickInfo, :hoverInfo, :interactorSettings, :pickingModes, :showOrientationAxes, :style, :triggerRender, :triggerResetCamera]
        wild_props = Symbol[]
        return Component("'vtk'_view", "View", "dash_vtk", available_props, wild_props; kwargs...)
end

'vtk'_view(children::Any; kwargs...) = 'vtk'_view(;kwargs..., children = children)
'vtk'_view(children_maker::Function; kwargs...) = 'vtk'_view(children_maker(); kwargs...)
